 ...
 * <li>重载 {@link jmp123.decoder.AbstractDecoder#fillBuffer(byte[], int, int)
 * fillBuffer} 方法，使解码器能够从输入流获取数据。</li>
 * <li>重载 {@link jmp123.decoder.AbstractDecoder#done() done}
 * 方法，解码结束时作必要的清理，例如关闭输入流等。</li>
 * </ol>
 * <p>
 * 
 * @version 0.400
...
public class MiniPlayer extends AbstractDecoder {
	...
	public String open(String name) throws IOException {
		mInputStream = new FileInputStream(name);
		return super.openDecoder();// 该方法调用了父类的nextFrameHeader,而nextFrameHeader调用了fillBuffer方法;
	}

	@Override
	protected int fillBuffer(byte[] b, int off, int len) {
		Log.v("jmp_test",
				"<fillBuffer>" + String.format("off=%d,size=%d", off, len));
		try {
